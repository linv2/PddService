...
<section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        <el-form :inline="true" :model="filters">
            <el-form-item>
                <el-button type="primary" size="small" @click="onPrint">打印</el-button>
            </el-form-item>
        </el-form>
    </el-col>
    ...
    <el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作">
            <template slot-scope="scope">
                <orderRowButton :order="scope.row" :index="scope.index" showBtns="['sync']"></orderRowButton>
            </template>
        </el-table-column>
    </el-table>
    ...
    <el-drawer :title="'订单打印('+this.multipleSelection.length+')'" :visible.sync="drawer">
        <printOrder :gridData="multipleSelection" @onCancel="onCancel">
        </printOrder>
    </el-drawer>
</section>
</template>
...
export default {
    ...
    watch: {
        "$store.state.socket.printResult": function (val) {
            if (val.taskStatus == "printed") {
                var successWaybill = [];
                for (var index in val.printStatus) {
                    let item = val.printStatus[index];
                    if (item.status == "success") {
                        let waybillCode = item.documentID;
                        successWaybill.push(waybillCode);
                    }
                }
                this.renderWaybilledData(successWaybill);
            }
        }
    },
    methods: {
